...
import { XMarkIcon, Bars3Icon, HomeIcon, UserIcon, ShoppingCartIcon } from '@heroicons/react/24/outline'
...
const ClienteLayout = () => {
  ...
  const navigation = [
    { name: 'Dashboard', href: '/cliente', icon: HomeIcon, current: location.pathname === '/cliente' },
    { name: 'Mis Pedidos', href: '/cliente/pedidos', icon: ShoppingCartIcon, current: location.pathname.includes('/cliente/pedidos') },
    { name: 'Mi Perfil', href: '/cliente/perfil', icon: UserIcon, current: location.pathname.includes('/cliente/perfil') },
  ]
      ...
      <div className={`fixed inset-0 z-40 flex md:hidden ${sidebarOpen ? 'visible' : 'invisible'}`} role="dialog" aria-modal="true">
        ...
        <div className={`relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-primary-700 transition ${sidebarOpen ? 'translate-x-0 ease-out duration-300' : '-translate-x-full ease-in duration-200'}`}>
          ...
          <div className="flex-shrink-0 flex items-center px-4">
            <h1 className="text-white font-bold text-xl">Mi Cuenta</h1>
          </div>
          ...
          <div className="mt-5 flex-1 h-0 overflow-y-auto">
            <nav className="px-2 space-y-1">
              ...
              >
                <HomeIcon className="mr-4 h-6 w-6 text-primary-300" aria-hidden="true" />
                Ir a Inicio
              </Link>
            </nav>
          </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-primary-700">
          <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
            ...
            <nav className="mt-5 flex-1 px-2 space-y-1">
              ...
              >
                <HomeIcon className="mr-3 h-6 w-6 text-primary-300" aria-hidden="true" />
                Ir a Inicio
              </Link>
            </nav>
          </div>
        ...
        <header className="bg-white shadow">
          <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 md:px-8 flex justify-between items-center">
            <h1 className="text-2xl font-semibold text-gray-900">
              {navigation.find(item => item.current)?.name || 'Mi Cuenta'}
            </h1>
